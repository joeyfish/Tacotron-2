...
_replacement_expression2 = [(re.compile('{}'.format(x[0]), re.IGNORECASE), x[1]) for x in [
    ('#0', ' '),
    ('#1', ' '),
    ('#2', ' '),
    ('#3', ' '),
...
_replacement_expression3 = [(re.compile('{}'.format(x[0]), re.IGNORECASE), x[1]) for x in [
    ('#0', '#1'),
]]
...
def replaceTag(content):
    for regex, replacement in _replacement_expression2:
        content = re.sub(regex, replacement, content)
    return content

def replaceTag2(content):
    for regex, replacement in _replacement_expression3:
        content = re.sub(regex, replacement, content)
    return content
...
def cn2pinyin(content):
    ...
    for txt in contents:
        ...
        result.append(txt)
        string = "".join(result)
    rets = process_phone(string).replace("  "," ")    
    rets = rets.replace("  "," ")
    return rets

def cn2pinyin2(content):
    contents = cn_format(content)
...
if __name__ == "__main__":
    ...
    content = cn2pinyin2(content)
    print(content)
    content = cn2pinyin(content)
    print(content)
    content = "而 对 楼市 成交 抑制 作用 最 大 的 限 购"
    content = cn2pinyin2(content)
    print(content)
    content = cn2pinyin(content)
    print(content)
